...
bid_map = {}
hands = []
cards = ["J"] + [str(i) for i in range(1, 10)] + ["T", "Q", "K", "A"]
...
def rank(hand: str):
    ...
    if counts["J"] >= 1:
        for i, c in enumerate(hand):
            if c == "J":
                return max([rank(hand[:i] + replace_card + hand[i + 1:]) for replace_card in cards[1:]])
...
for hand in hands:
    curr_strength = rank(hand)
    hand_ranks[curr_strength].append(hand)
